    ...
    def validate_epoch(self, val_iter):
        ...
        with torch.no_grad():
            for batch in pbar:
                ...
                loss = sequence_loss(outputs, batch[-1][:,1:], pad_id=self.pad_id)
...
class TeacherForcing(Trainer):
    def train_step(self, src_batch, tgt_batch):
        ...
        outputs, _ = self.model(src_batch, tgt_batch)
        loss = sequence_loss(outputs, tgt_batch[:,1:], pad_id=self.pad_id)
